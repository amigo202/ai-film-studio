    ...
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-40 transition-all duration-300 ${
          isScrolled
            ? 'bg-[#09090b]/95 backdrop-blur-md border-b border-white/10 py-3 sm:py-4'
            : 'bg-gradient-to-b from-[#09090b]/90 to-transparent py-4 sm:py-6'
        }`}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-12 flex items-center justify-between">
          ...
          >
            <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-md bg-gradient-to-br from-amber-500/20 to-amber-600/10 border border-amber-500/30 flex items-center justify-center text-amber-400 group-hover:border-amber-400/60 transition-colors shrink-0">
              <Film className="w-4 h-4" />
            </div>
            <div className="flex flex-col text-right">
              <span className="font-syne text-base sm:text-lg font-bold tracking-wider text-white uppercase flex items-center gap-1.5 leading-tight">
                AmitAI
                <span className="inline-block w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse"></span>
              </span>
              <span className="text-[9px] sm:text-[10px] tracking-widest text-zinc-400 uppercase font-mono leading-tight">
        ...
        {/* Mobile Menu Overlay */}
        {mobileMenuOpen && (
          <div className="md:hidden bg-[#09090b]/98 backdrop-blur-2xl border-b border-white/10 px-6 py-6 animate-fade-in font-hebrew text-right">
            <div className="flex flex-col gap-3">
              {navLinks.map((link) => (
                <Link
                  key={link.to}
                  to={link.to}
                  className="text-base font-bold text-white hover:text-amber-400 transition-colors py-2.5 border-b border-white/5"
                >
                  {link.label}
                </Link>
              ))}
              ...
              <Link
                to="/contact"
                className="flex items-center justify-center gap-2 text-sm font-bold bg-amber-400 text-black py-3.5 rounded-xl mt-1"
              >
                <span>התחלת פרויקט</span>
